class RxnconsoController < ApplicationController
  ...
  def suggestions
    ...
    @items = Rxnconso.where("STR like ? and TTY in ('PSN', 'SCD','SY')", "%#{params[:term]}%").limit(10).collect{|x| x.STR.humanize.gsub(/\b('?[a-z])/) { $1.capitalize }}.uniq
    render :text => @items
  end
  ...
  def map_drug
    ...
    if request.post?
      map_to = Rxnconso.where("STR = ? and TTY in ('PSN', 'SCD','SY')", "#{params[:rxnconso][:matching_drug]}").first
      if  map_to.blank?
